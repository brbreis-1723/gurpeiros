# Magia do Limiar e Itens de Poder

> *"A magia jamais foi criada para ser suportada pela carne. O corpo do mago é apenas a última barreira entre a vontade e o caos. Os sábios aprenderam, há muito tempo, que pedra, metal e cristal suportam melhor esse fardo do que qualquer mortal. É por isso que os maiores magos jamais caminham desarmados de seus artefatos."*
>
> — **Fragmento atribuído ao Primeiro Arquiteto**

## Introdução: Os Itens de Poder

Toda magia gera **Tensão Mágica**, e todo conjurador possui um limite para suportá-la antes que ocorram consequências catastróficas. Os **Itens de Poder** foram criados justamente para ampliar esse limite.

Um Item de Poder é um objeto especialmente preparado para servir como um **reservatório arcano**, absorvendo parte da Tensão Mágica produzida por seu portador. Diferentemente de armas ou armaduras encantadas, um Item de Poder não aumenta a habilidade do mago nem melhora seus feitiços; sua função é permitir que ele canalize quantidades muito maiores de energia antes de atingir seu próprio Limiar.

Em Zandia, os maiores tesouros não são aqueles capazes de destruir cidades, mas aqueles capazes de suportar o peso da magia sem se despedaçar.

!!! info "O que pode se considerado um **Item de Poder**?"
    Itens de Poder são objetos especialmente preparados para armazenar e canalizar energia arcana. Apenas determinados tipos de objetos podem receber esse encantamento, como cajados, cetros, varinhas, joias (anéis, amuletos, broches, brincos, braceletes, tiaras e adornos semelhantes).
    
    Além da forma, a matéria-prima do objeto é essencial. Um Item de Poder deve ser confeccionado a partir de materiais orgânicos, como madeira, osso, marfim e outros materiais naturais equivalentes, ou de pedras preciosas. Quando possuir partes metálicas, estas devem ser feitas exclusivamente de ouro, prata ou platina. Esses materiais possuem as propriedades necessárias para conduzir e estabilizar as energias arcanas.
    
    Objetos destinados principalmente ao combate ou à proteção, como armas, armaduras, escudos e vestimentas, não possuem as propriedades necessárias para desempenhar essa função e, portanto, não podem ser transformados em Itens de Poder.


---

## **O Ritual de Sintonia**

Todo mago tem os conhecimentos necessários para realizar o ritual:

- Um personagem pode estar sintonizado com **apenas um Item de Poder** por vez.
- O ritual exige 10 minutos preparando um circulo mágico e, ao final dele, um teste contra a skill **Ritual Magic**.
- Ele requer o gasto de **$100** em ingredientes e componentes como velas, incensos, etc.
- O Ritual de Sintonia ocupa um dia inteiro (aproximadamente **12 horas** de concentração, podendo parar apenas para comer, fazer as necessidades básicas e dormir). Durante esse ritual, o objeto é impregnado pela essência mágica de seu portador.
- Se o mago for interrompido em sua concentração, precisará iniciar novamente o ritual, sem prejuizo para o circulo preparado ou para os ingredientes utilizados. Ainda assim, precisará fazer novamente um teste contra **Ritual Magic**.
- Ao concluir uma nova sintonia, qualquer vínculo anterior é imediatamente encerrado.

---

## Qualidade dos Itens de Poder

Todo Item de Poder possui um **Nível de Qualidade**, definido por seu **Valor Intrínseco**. Considere apenas o valor mundano do objeto, desconsiderando encantamentos e quaisquer propriedades mágicas. O Valor Intrínseco representa o investimento mínimo necessário para adquirir o item pronto ou, alternativamente, o custo dos materiais e componentes para confeccioná-lo.

O Valor Intrínseco é obtido aplicando o multiplicador correspondente ao Nível de Qualidade sobre a renda média do cenário. Por exemplo, em um cenário de NT 3 (renda média de $1.000), um item de qualidade **Superior** (4×) possui Valor Intrínseco mínimo de $4.000.

Se o valor de mercado de um objeto estiver entre dois Níveis de Qualidade, utilize sempre o nível inferior. Por exemplo, em um cenário de NT 2, um anel de ouro mundano adquirido por $1.000 situa-se entre **Padrão** ($750) e **Superior** ($3.000); portanto, ele é considerado um item de qualidade **Padrão**.


O nível de qualidade determina duas características fundamentais:

- Limiar do Item;
- Resistência à Sobrecarga (HT).


<table>
<thead>
<tr><th>Qualidade</th><th>Valor intrínseco (Multiplicador)</th><th>Limiar do Item</th><th>HT</th></tr>
</thead>
<tbody>
<tr><td>Padrão</td><td>1x</td><td>+5</td><td>12</td></tr>
<tr><td>Superior</td><td>4x</td><td>+10</td><td>12</td></tr>
<tr><td>Excepcional</td><td>10x</td><td>+15</td><td>13</td></tr>
<tr><td>Magistral</td><td>25x</td><td>+25</td><td>14</td></tr>
<tr><td>Obra-Prima</td><td>100x</td><td>+50</td><td>15</td></tr>
<tr><td>Lendário</td><td>Inestimável</td><td>+100</td><td>16</td></tr>
</tbody>
</table>


!!! info "Itens Lendários"
     Itens Lendários não podem ser confeccionados por meios comuns e não possuem um preço de mercado definido. São relíquias da Era Anterior, armas dos Arquitetos, presentes “divinos” ou artefatos perdidos há séculos. 
     
     Sua raridade e singularidade fazem com que não existam lojas, artesãos ou comerciantes especializados capazes de simplesmente oferecê-los à venda. Mesmo um governante não poderia simplesmente encomendar um Item Lendário ou adquiri-lo pelo preço de uma mercadoria comum. 
     
     Isso, entretanto, não significa que um Item Lendário jamais possa mudar de mãos por meio de uma negociação. Um artefato pode ser vendido, trocado ou utilizado como moeda de barganha, mas seu valor dependerá inteiramente das circunstâncias e das pessoas envolvidas. Para alguns, pode valer uma fortuna; para outros, seu verdadeiro valor pode ser completamente desconhecido. 
     
     A obtenção de um Item Lendário normalmente exige descobri-lo, herdá-lo, conquistá-lo ou recebê-lo como recompensa por feitos extraordinários. Contudo, um personagem que encontre uma dessas relíquias pode decidir vendê-la ou trocá-la, caso não reconheça seu verdadeiro valor, não tenha utilidade para ela ou simplesmente prefira a riqueza que ela pode proporcionar. 
     
     Essa decisão, porém, raramente é simples. Itens Lendários atraem atenção. A notícia de que uma dessas relíquias foi encontrada pode despertar a cobiça de criminosos, colecionadores, nobres, governantes e outros magos. Os próprios Arquitetos podem desejar reivindicar um artefato relacionado à sua história ou ao seu poder. Um comprador pode tentar roubá-lo em vez de pagar por ele; um governante pode tentar confiscá-lo; e uma organização pode considerar sua posse uma ameaça que não pode ser tolerada. 
     
     Ao longo da história, guerras já foram travadas por Itens Lendários. Cidades foram destruídas, alianças foram rompidas e dinastias chegaram ao fim pela posse de uma única relíquia. Por isso, possuir um Item Lendário pode ser tão perigoso quanto cobiçá-lo. O proprietário não precisa apenas decidir quanto o artefato vale: precisa decidir se consegue protegê-lo, em quem pode confiar e quem poderá descobrir que ele está em sua posse. 
     
     Um personagem que decida vender ou trocar um Item Lendário também assume riscos. A negociação pode atrair a atenção de pessoas poderosas, revelar a localização do artefato ou colocar o vendedor no centro de uma disputa muito maior do que ele poderia imaginar. O comprador pode tentar tomar o item à força, oferecer uma recompensa falsa ou simplesmente concluir que eliminar o vendedor é mais fácil do que cumprir o acordo. 
     
     Assim, um Item Lendário não é apenas uma riqueza. É uma fonte de poder, cobiça e perigo. Um personagem pode decidir mantê-lo, vendê-lo, trocá-lo, escondê-lo ou entregá-lo a alguém capaz de protegê-lo. Nenhuma dessas escolhas é necessariamente correta — mas todas podem ter consequências. 
     
     > Às vezes, a maior dificuldade não é encontrar um Item Lendário. É sobreviver depois que todos descobrem que você o encontrou.

---

## Acúmulo de Tensão

O Item de Poder funciona como um reservatório arcano que preserva seu portador dos efeitos imediatos da sobrecarga mágica.

Sempre que um feitiço gerar Tensão Mágica, o conjurador deverá decidir onde essa Tensão será acumulada.

1. O mago pode optar por acumular a Tensão Mágica em seu próprio **Limiar** ou no **Limiar do Item de Poder**.
2. A Tensão acumulada no **Item de Poder** <u>não é contabilizada no Limiar do mago</u> e, portanto, **não exige Testes de Calamidade**. Os Limiares do mago e do Item de Poder são controlados de forma totalmente independente.
3. Caso a Tensão acumulada ultrapasse o Limiar do Item de Poder, ele entra imediatamente em **Sobrecarga**.
4. Uma vez escolhida a origem da Tensão gerada por um feitiço, ela não pode ser redistribuída posteriormente entre o mago e o Item de Poder.

---

## Sobrecarga

Em situações desesperadoras, um mago pode continuar canalizando magia mesmo após esgotar completamente o Limiar de seu Item de Poder.

Sempre que a Tensão acumulada em um Item de Poder ultrapassar seu Limiar, o item deve realizar imediatamente um **Teste de HT**, utilizando o valor correspondente à sua qualidade.

!!! warning "Atenção!"
     Enquanto permanecer em **Sobrecarga**, o Item de Poder deverá realizar um **novo Teste de HT** sempre que seu portador lançar qualquer feitiço, independentemente de a Tensão gerada ser acumulada no Item ou no próprio mago.<br>

### Resultados do teste de Sobrecarga

Se o item passar no teste de HT:

!!! success "Sucesso!"
     O Item suporta a sobrecarga sem sofrer danos.<br> Nada acontece... por enquanto!<br>
     
---

Se o item falhar no teste de HT:

!!! failure "Falha (Normal)!"
     O Item sofre a perda de 1 PV por dano estrutural na sua matriz arcana (ignorando qualquer RD). Além disso, deve rolar 3d6 verificando o resultado na **Tabela de Sobrecarga** a seguir:<br><br>
     **Tabela de Efeitos de Sobrecarga (role 3d6):**
     <table>
     <thead>
     <tr><b><th>3d6</th><th>Resultado</th><th>Consequência</th></b></tr>
     </thead>
     <tbody>
     <tr><td>3</td><td>**Aniquilação Arcana**</td><td>A matriz arcana entra em colapso absoluto. O Item perde permanentemente a capacidade de funcionar como item de poder além de todas as propriedades mágicas que tenha tornando-se um objeto mundano. Ele não pode mais ser sintonizado!</td></tr>
     <tr><td>4–5</td><td>**Ressonância Descontrolada**</td><td>A matriz arcana sofre uma mutação imprevisível. Enquanto estiver sintonizado com o item, o mago sofre uma penalidade  de **-3 em todas as magias que realizar**. Essa penalidade somente é removida  quado o mago realizar novamente o **Ritual de Sintonia**, seja com o mesmo objeto ou com outro diferente. Além disso o Mestre pode acrescentar um pequeno efeito narrativo permanente (luz constante, sussurros, calor, frio, alteração de cor, etc.).</td></tr>
     <tr><td>6–8</td><td>**Instabilidade Arcana**</td><td>Até o próximo amanhecer, o Limiar máximo do Item é reduzido pela metade (arredondando para baixo) e todos os Testes de HT realizados por ele sofrem penalidade de -2. Se esse resultado se repetir no mesmo dia, acumule +1 dia de **Instabilidade Arcana**.</td></tr>
     <tr><td>9–11</td><td>**Fissuras Rúnicas**</td><td>As inscrições mágicas são parcialmente destruídas. O Item não recupera seu Limiar no próximo amanhecer. A recuperação automática retorna normalmente no amanhecer seguinte. Se esse resultado sair novamente no mesmo dia, acumule +1 dia sem recuperar seu limiar. </td></tr>
     <tr><td>12–14</td><td>**Quebra de sintonia**</td><td>A matriz arcana do item é abalada e a sintonia com o mago é quebrada. O mago precisará fazer novamente um **Ritual de Sintonia** para que o item funcione novamente.</td></tr>
     <tr><td>15–17</td><td>**Cicatriz Arcana**</td><td>O Item desenvolve uma Cicatriz Arcana permanente. Role na **Tabela de Cicatrizes Arcanas**.</td></tr>
     <tr><td>18</td><td>**Explosão de Mana**</td><td>Toda a Tensão armazenada é liberada instantaneamente. O portador recebe 4d6 de dano pela explosão (a RD protege normalmente) e sofre imediatamente uma Calamidade como se toda essa Tensão estivesse acumulada em seu próprio Limiar. Como consequência positiva, o item fica completamente descarregado: deixa de estar em Sobrecarga e sua Tensão é reduzida a zero.</td></tr>
     </tbody>
     </table>

---

Se for uma **Falha Crítica**...

!!! danger "Falha Crítica!"
     Indica um colapso violento da matriz arcana do Item de Poder. Nessa situação, o item sofre dano massivo estrutural e é destruído!

---

## Cicatrizes Arcanas

As **Cicatrizes Arcanas** representam danos permanentes na estrutura metafísica de um Item de Poder.

Diferentemente dos danos físicos, elas não podem ser reparadas por meios convencionais. Sua remoção exige rituais complexos, materiais raríssimos ou a intervenção de um mestre artesão arcano, quando isso for sequer possível.

Um Item pode acumular diversas Cicatrizes Arcanas ao longo de sua existência, tornando-se um testemunho vivo dos grandes feitos — e dos grandes excessos — de seus antigos portadores.

### **Tabela de Cicatrizes Arcanas**

Role 1d6:
<table>
<thead>
<tr><th>1d6</th><th>Cicatriz</th><th>Efeito</th></tr>
</thead>
<tbody>
<tr><td>1</td><td><b>Matriz Fragilizada</b></td><td>O HT do Item é reduzido permanentemente em 1. Se esse resultado for acumulado ao ponto da HT dele chegar a zero o item é destruido!</td></tr>
<tr><td>2</td><td><b>Capacidade Comprometida</b></td><td>O Limiar máximo do Item é reduzido **permanentemente** em 5 pontos. Se o Limiar chegar a zero ele deixa de ser um item de poder!</td></tr>
<tr><td>3</td><td><b>Fluxo Instável</b></td><td>A primeira magia canalizada através do Item a cada amanhecer exige imediatamente um Teste de HT, mesmo que o Item esteja abaixo de seu Limiar.</td></tr>
<tr><td>4</td><td><b>Ressonância Arcana</b></td><td>-1 adicional nos Testes de HT quando acima do Limiar.</td></tr>
<tr><td>5</td><td><b>Marca dos Excessos</b></td><td>O Item adquire uma alteração permanente visível (runa apagada, cristal rachado, metal escurecido, murmúrios, brilho irregular, calor constante etc.). Não produz efeito mecânico, mas revela sua história e pode influenciar interações narrativas.</td></tr>
<tr><td>6</td><td><b>Ferida Profunda</b></td><td>Role duas vezes nesta tabela, ignorando novos resultados 6. Ambos os efeitos são cumulativos.</td></tr>
</tbody>
</table>

---

## Dissipação da Tensão nos itens de Poder

Ao amanhecer, um Item de Poder dissipa parte da Tensão Mágica acumulada em sua matriz arcana. Diferentemente do Limiar do mago, cuja recuperação é gradual e previsível, os artefatos dependem de um processo espontâneo de estabilização da energia mágica.

Na maioria das vezes, apenas uma pequena fração da energia acumulada consegue ser dissipada naturalmente. Recuperações mais expressivas são incomuns e representam momentos em que a matriz arcana encontra, por breve período, um raro estado de equilíbrio.

Assim, ao amanhecer, para cada Item de Poder, role 3d6 para determinar quanto de seu Limiar será recuperado.
<table>
    <thead>
        <tr>
            <th>Resultado (3d6)</th>
            <th>Recuperação</th>
        </tr>
    </thead>
    <tbody>
        <tr><td><strong>3</strong></td><td>Recupera <strong>100%</strong> do Limiar.</td></tr>
        <tr><td><strong>4</strong></td><td>Recupera <strong>50%</strong> do Limiar.</td></tr>
        <tr><td><strong>5-6</strong></td><td>Recupera <strong>30%</strong> do Limiar.</td></tr>
        <tr><td><strong>7-8</strong></td><td>Recupera <strong>20%</strong> do Limiar.</td></tr>
        <tr><td><strong>9-12</strong></td><td>Recupera <strong>10%</strong> do Limiar.</td></tr>
        <tr><td><strong>13-14</strong></td><td>Recupera <strong>20%</strong> do Limiar.</td></tr>
        <tr><td><strong>15-16</strong></td><td>Recupera <strong>30%</strong> do Limiar.</td></tr>
        <tr><td><strong>17</strong></td><td>Recupera <strong>50%</strong> do Limiar.</td></tr>
        <tr><td><strong>18</strong></td><td>Recupera <strong>100%</strong> do Limiar.</td></tr>
    </tbody>
</table>

!!! note "Nota sobre a Recuperação:"
     A recuperação de um Item de Poder não representa sua capacidade máxima, mas sim a eficiência com que sua matriz arcana conseguiu dissipar a energia acumulada durante a noite.
     
     Na maioria dos dias, essa dissipação é limitada, resultando na recuperação de apenas 10% do Limiar. Em raras ocasiões, entretanto, a estrutura arcana entra em perfeita ressonância com o fluxo natural da magia, permitindo recuperações muito superiores.
     
     Essa distribuição é propositalmente assimétrica:
     <ul>
     <li><strong>10%</strong> representa o comportamento esperado da maioria dos artefatos.</li>
     <li><strong>20% e 30%</strong> representam recuperações acima da média.</li>
     <li><strong>50%</strong> é um evento raro.</li>
     <li><strong>100%</strong> representa uma recuperação extraordinária, ocorrendo em menos de <strong>1%</strong> dos amanheceres.</li>
     </ul>

     <div style="max-height:600px; overflow-y:auto;" align="center">
     <table>
    <thead>
        <tr>
            <th>Recuperação</th>
            <th>Resultados (3d6)</th>
            <th>Probabilidade</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td><strong>Recupera 10% do Limiar</strong></td>
            <td>9–12</td>
            <td>70,37%</td>
        </tr>
        <tr>
            <td><strong>Recupera 20% do Limiar</strong></td>
            <td>7–8 ou 13–14</td>
            <td>18,52%</td>
        </tr>
        <tr>
            <td><strong>Recupera 30% do Limiar</strong></td>
            <td>5–6 ou 15–16</td>
            <td>7,41%</td>
        </tr>
        <tr>
            <td><strong>Recupera 50% do Limiar</strong></td>
            <td>4 ou 17</td>
            <td>2,78%</td>
        </tr>
        <tr>
            <td><strong>Recupera 100% do Limiar</strong></td>
            <td>3 ou 18</td>
            <td>0,93%</td>
        </tr>
     </tbody>
     </table></div>



----

## Filosofia da Regra

Os Itens de Poder não tornam um mago apenas mais poderoso; eles tornam possível que ele **ouse mais**.

Enquanto um conjurador comum deve administrar cuidadosamente sua própria Tensão Mágica, um grande artefato funciona como um reservatório que absorve esse fardo, permitindo feitos extraordinários. Contudo, nenhum recipiente é infinito. Todo Item de Poder possui um ponto de ruptura, e cada vez que um mago decide ultrapassá-lo, coloca em risco um tesouro que talvez jamais possa ser substituído.

As **Cicatrizes Arcanas** representam essa história. Cada rachadura, runa apagada ou cristal partido é a lembrança de um momento em que um mago escolheu desafiar os limites da própria magia. Assim, os maiores artefatos de Zandia não são apenas ferramentas de poder: são relíquias vivas, marcadas pelos séculos e pelos sacrifícios de todos aqueles que as empunharam antes.
